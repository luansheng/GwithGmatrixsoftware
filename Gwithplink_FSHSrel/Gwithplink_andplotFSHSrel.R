Gwithplink <- function(genofile,genoformat,outname){
  cat('\n... Computing G matrix with plink ...\n')
  if(genoformat=='binary'){
    system(paste('plink.exe --allow-no-sex --nonfounders --chr-set 30 --bfile ',genofile,' --make-rel square --out ',outname,sep=''))
  } else if(genoformat=='pedmap'){
    system(paste('plink.exe --allow-no-sex --nonfounders --chr-set 30 --file ',genofile,' --make-rel square --out ',outname,sep=''))
  }
  
  cat('\n... Exporting orginal and recoded IDS ...\n')
  id <- read.table(paste(outname,'.rel.id',sep=''),stringsAsFactors=F)
  id <- cbind(IID=id[,2],IIDNr=1:nrow(id))
  write.table(id,paste(outname,'.id',sep=''),quote=F,row.names=F,col.names=F)
  
  cat('... Exporting G in asreml format ...\n')
  G <- read.table(paste(outname,'.rel',sep=''),stringsAsFactors=F)
